...
- **枚举是下发的，不是复制的。** `GET /api/legal/validity-statuses` 返回
  ...
  ——`0` 未标注、`-1` 已失效。
- **控件是一排状态标签，不是下拉菜单。** 点一个标签切换「该状态是否展示」；一个都不选就是全部
  展示，由最前面的「全部」标签表达这个状态。把选项和当前选择一起藏在多一次点击之后，在一个
  专职"按状态看文件"的页面上是错的形状。
  标签靠左摆，并且根目录**不渲染**面包屑：「全部文件」和「全部」标签说的是同一件事，一个状态
  两个标签只会让人猜哪个才是筛选。进入文件夹后面包屑回来——在那里它是导航，不是标题。
