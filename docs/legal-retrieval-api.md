...
| `include_invalid` | bool | ✗ | `false` | 是否包含**已废止**（`validity_status=1`）与**已失效**（`-1`）的法条。默认排除二者，只返回仍有法律效力的条文；置 `true` 可查历史版本（如按行为发生时的法律）。**只作用于语义召回**：`match_mode=exact` 与法条详情按 ID 点名取，不受影响 |
...
| `filtered_invalid_count` | int | 默认口径下被排除的已废止/已失效条数（`include_invalid=true` 时恒为 0） |
...
| `validity_status` | 效力状态原始整数（枚举见上，服务端原样下发；默认口径已在召回侧排除 `1` 已废止与 `-1` 已失效） |
| `province` / `city` | 省 / 市 |
...
值由数据源字典给出，服务端**原样下发**（不做归一化、不解释）：

| 值 | 含义 | 默认口径 |
|---|---|---|
| `3` | 现行有效 | **返回** |
| `2` | 已修改（该版已被后续版本取代） | **返回**（库里能拿到的最新版本） |
| `0` | 未标注（主要是"修改、废止的决定"这类文件） | **返回**（本身是有效文件） |
| `4` | 尚未生效（还没到施行日期的新法） | **返回** |
| `1` | 已废止 | **排除**（`include_invalid=true` 时返回） |
| `-1` | 已失效 | **排除**（`include_invalid=true` 时返回） |

被排除的条数在响应里以 `filtered_invalid_count` 回显——"为什么只返回 3 条"是这个开关最容易
引起的疑问。元数据里**没有** `validity_status` 键的条目视为有效并返回（读不到不等于失效）。

> 过滤发生在召回之后（`validity_status` 目前不是 Milvus 标量字段），因此服务端会放大约 4 倍
> 候选量来补偿。若某次查询的候选大量集中在已废止法条上，返回条数仍可能少于 `top_k`。
> 把该字段做成 Milvus 标量做预过滤需要重建集合，属于后续项。
...
| `validity_status` | 效力状态 | 先看它——默认口径下已废止/已失效的本来就不会返回 |
...
后两条在当前默认口径下会被排除（`validity_status=1`）；这里保留是为了说明**同为「第一条」，
日期能把三个版本排出先后**。同法名的新旧版本、基础法与针对它的修改决定，同理。
...
| 能力 | 状态 |
|---|---|
| 仅返回现行有效法条（PRD F-103） | **部分实现**：默认已排除 `1` 已废止与 `-1` 已失效；若要"只留 `3` 现行有效"（连未标注/已修改/尚未生效一起排除），`include_invalid` 表达不了，需要显式的状态白名单参数（后续项） |
