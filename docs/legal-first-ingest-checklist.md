...
- [x] **已确认 `validity_status` 枚举语义**（2026-09-15，取自数据源字典）：

  | 值 | 含义 | 全语料份数 | 入库清单份数 |
  |---|---|---|---|
  | `3` | 现行有效 | 17,235 | 17,192 |
  | `2` | 已修改 | 7,473 | 159 |
  | `1` | 已废止 | 3,068 | 2,581 |
  | `-1` | 已失效 | 40 | 36 |
  | `4` | 尚未生效 | 49 | 37 |
  ...
  两点更正（此前按语料反推时为 `0`/`-1` 给过错读）：`0` 不是"已废止"而是**未标注**——
  它主要落在「修改、废止的决定」这类文件上（1,844/2,031），源库本就不给这类文件标状态；
  `-1` 不是"未标注"而是**已失效**。选版规则"组内优先取 `3`"因此得到证实，可以从假设转为事实。
  检索侧默认排除 `1` 与 `-1`（见方案 §6.1）。
- [ ] 选版规则与人工队列处置：379 组无 `status=3`、2 组日期并列、1,984 份孤本 `status=0`
      （多为废止决定，正文本身有价值，不能仅凭状态删除）。现有规则（组内优先 `3`、再按发布
      日期取新）继续使用，**它依赖的假设已被官方枚举证实**（`3` 就是现行有效）。剩下待处置的
      是例外组：379 组无 `3`（整组都是已废止/已失效/尚未生效/未标注），以及 `0.7%` 的
      "一组两条 `3`"、`0.3%` 的"`2` 比 `3` 新"这类源库自身不自洽的情况。
