...
> **2026-09-15 更新（两次）**：先是因"`validity_status` 枚举定义拿不到"决定 F-103 不做；
> 随后**该定义在数据源字典里找到了**（`3` 现行有效 / `2` 已修改 / `1` 已废止 / `-1` 已失效 /
> `4` 尚未生效 / `0` 未标注，见
> [legal-first-ingest-checklist.md](legal-first-ingest-checklist.md) 第四节），前提不再成立。
>
> **改为：默认排除 + 参数放开。** 入库口径不变——指示明确是"入库不受影响"，已废止/已失效的
> 法条照收（共约 12%），保留全集也保住了"行为时法"这类查询。**检索侧默认排除 `1` 已废止与
> `-1` 已失效**，新增请求字段
> `include_invalid`（默认 `false`）可放开查全部；被剔除条数由响应的 `filtered_invalid_count`
> 回显。`0` 未标注（主要是决定类文件）与 `4` 尚未生效**不在排除集里**。
>
> 注：过滤发生在召回之后（该字段尚未做成 Milvus 标量），服务端按约 4 倍放大候选量补偿；
> 做成标量做预过滤需要重建集合，列为后续项。
...
| 仅返回有效法条 | **部分满足** | 默认口径已排除 `1` 已废止与 `-1` 已失效（实测：已废止文档默认不出现、`include_invalid=true` 时出现）；若要"只留 `3` 现行有效"，还需显式的状态白名单参数 |
...
| 有效性 | **部分满足** | 同上：默认排除已废止/已失效；`0` 未标注与 `4` 尚未生效仍会返回 |
