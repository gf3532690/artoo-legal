...
### 2.1 上线语料（29,957 份爬取 docx）实测

爬取语料与本节的 347 份样本**不是同一批数据**，容量结论随之翻转。以下数字来自
`app/scripts/audit_legal_metadata.py`：全量 `fast` 核对（29,957 份，0 异常，约 78 秒）
与 300 份 `full` 抽样（走真实 chunker 与大小护栏）。

| 项 | 实测 |
|---|---|
| 文件数 / 文字量 | 29,957 份 `.docx` / 约 1.68 亿字（解压 701 MB） |
| 行首 `第X条` 总数 | **1,141,002**（中位 38 条/篇，P90 65） |
| 完全无行首 `第X条` 的文档 | **3,227 份（10.8%）**，其中 1,853 份是「修改、废止的决定」 |
| `law_type` | 地方法规 25,285 / 修改废止决定 1,902 / 司法解释 846 / 行政法规 812 / 法律 477 / 其余少量 |
| `validity_status` | `3`→17,235、`2`→7,473、`1`→3,068、`0`→2,092、`4`→49、`-1`→40（枚举语义仍待数据源确认） |
| `effective_date` | 属性 27,495 份，其余 2,462 份为空（属性 + 正文兜底覆盖率 91.8%） |
| 同名多版本 | 22,036 个法名、6,108 组多版本，按选版规则可淘汰 **7,921 份**（占条文行 31%） |
| 目录行 | 含独立「目录」行的文档仅 0.4%（样本语料是 76.3%），D10 在本语料上几乎不触发 |

**容量结论翻转。** 2000 份 `full` 抽样（`--mode full --sample 2000 --seed 11`，按
「有条文 / 无条文」分层后套用到全量）测得 `chunk/条 = 2.43`（条文按「款」被拆成多个
子块）、无条文文档 `chunk/份 = 32.5`（按行切）。抽样与全量的结构分布一致：无条文文档
占比 10.9%（全量 10.9%），中位 38 条/篇（全量 38）。同一批抽样还给出了对照口径——
「一条文一子块」时 `子块/条 = 1.01`、无条文文档 `10.8`，即**切分粒度本身贡献了 2.4 倍
的体量差**。按此推算：

| 方案 | 估算 child chunk | 稠密向量原始占用（dim=1024 float32） | 对 `kb_chunk_cap`（1,000,000） |
|---|---|---|---|
| 现状（条文按款切子块）· 全部入库 | **2.88 M** | 11.8 GB | 超约 2.9 倍 |
| 现状 · 版本选版后 | **1.99 M** | 8.2 GB | 超约 2.0 倍 |
| 一条文一子块 · 全部入库 | **1.19 M** | 4.9 GB | 超约 1.2 倍 |
| 一条文一子块 · 版本选版后 | **0.82 M** | 3.4 GB | **在限内（余量 18%）** |

按 `kb_chunk_cap` 是**每个知识库**的口径，若保持现状切分不改，另一个选择是拆库：
拆成 3 个全局库时每库约 0.96 M（选版后 0.66 M）；拆 4 个每库约 0.72 M（选版后 0.50 M）。
代价是检索侧从"一个全局库"变成"一组全局库"，`api/retrieval.py` 与引导步骤都要跟着改。

因此**不能沿用「约 2.78 万 chunk、占上限 2.8%」的判断**——那是 347 份样本的数，不是
上线语料的数。上量前必须三选一：**改子块切分策略**（收益最大：2.88 M → 1.19 M）、
**按法条类别拆库**（不改算法，但要改检索侧的库集合逻辑）、或**调大 `kb_chunk_cap`**
（最省事，代价是 2.88 M 向量约 11.8 GB 稠密原始占用与相应索引内存）。版本选版能省约
31%，单独不足以解决问题，但它是唯一**不需要动算法或契约**的杠杆，建议无论选哪条路都先做。

> 「改粒度」的具体形态建议是**只对超长条文再切**：让 `LawsChunker` 一条文产一个子块，
> 超过 `child_chunk_size`（450 字）的由既有 `enforce_size_limits` 护栏按句子边界再切。
> 多数条文（平均 123 字）本就不该被拆开，长条文仍保持细粒度检索——上面 1.19 M/0.82 M
> 两行就是按这个口径算的，不是简单的"父块即子块"。

这个开关已经落地：KB config 支持 `law_child_policy`，取值 `paragraph`（默认，改造前行为）
与 `article`（一条文一子块）。**默认保持 `paragraph`**，因为"款级 vs 条文级"对召回质量
的影响在本仓库无法用数据判定——评测集是暂缓项，且本机没有 Milvus / 远程 Embedding /
Rerank 环境可跑对比。决策与 A/B 步骤见
`.agents/notes/proposed/architecture/2026-09-14-legal-child-chunk-granularity.md`。

**89 份文档（0.30%）曾无法入库，现已全部修复。** 成因由静态检测筛出，再逐份跑真实
`DocxLoader` 复核（打不开的 51 份全部复现失败，0 误报、0 漏报）：

| 成因 | 份数 | 现象 | 来源聚集 |
|---|---|---|---|
| `_rels/.rels` 声明 `userCustomization/customUI.xml` 但该部件不存在 | 44 | python-docx 抛错，文档直接失败 | 全部是江西省（省本级 + 南昌 / 景德镇 / 萍乡 / 新余 / 抚州 / 九江 / 赣州 / 赣南） |
| 内容类型是 macro-enabled（`.docm` 改名成 `.docx`） | 7 | python-docx 抛错 | 松原市 6 + 山西省 1 |
| 正文全部放在 Word 表格里 | 37 | `doc.paragraphs` 为空 → 文本为空 → OCR 关闭时判失败 | 吉林省 32 + 黑龙江省 4 + 四川省 1 |
| 正文全部放在文本框里 | 1 | 同上网状结构，`doc.paragraphs` 同样取不到 | 清远市城市市容和环境卫生管理条例（49 条 / 9,769 字） |

成因按来源地聚集，说明是各来源的序列化工具差异，不是随机损坏。按版本选版过滤后仍需
抢救的是 **45 份 / 45 个法名**（不含文本框那 1 份）——不处理会**整条法条缺失**，其中
包含江西省的大批法规。

处置（已实施，见 `Implemented/bug-fix/2026-09-14-docx-loader-tolerates-damaged-and-table-bodies`）：
`DocxLoader` 打开失败时先对包做内存自愈（删悬空关联声明、把 macro-enabled 主部件内容
类型改回标准 docx），再按文档顺序把表格文字（含嵌套表格）纳入正文，结构化遍历一无所获
时退回按 XML 取段落（覆盖文本框）。复检结果：**89 份全部可加载，0 失败，0 空正文**；
表格类文档从 0 字符恢复到 204,251 字符 / 1,649 条，文本框文档从 0 恢复到 9,769 字符 /
49 条。2000 份 `full` 抽样里"产出完整 chunk 的文档"从 1999/2000 变成 **2000/2000**。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `law_name` | str \| None | 法律全名；**优先取 docx 内嵌属性**，取不到才用「首行至日期行之前拼接」（见 D13；不能简单取首行） |
...
| `issuing_authority` | str \| None | 发布机关；**优先取 docx 属性**，取不到才从正文括注解析 |
| `publish_date` | str \| None | 该文件所载版本的发布日期；**优先取 docx 属性**，取不到才从正文括注解析 |
| `has_toc` | bool | **本次入库是否剥离了目录区**（标记驱动，见 D10；供排查） |
| `extraction_method` | str | rule / manual |
| `confidence` | float | 抽取置信度 |
| `effective_date` | str \| None | 施行日期（docx 属性优先，正文「自…起施行」兜底；两者都没有则为 `None`） |
| `validity_status` | int \| None | 时效状态原值（docx 属性）；**只落库，不解释枚举含义、不参与检索** |
| `law_type` | str \| None | 法律类别（`法律` / `行政法规` / `地方法规` / `司法解释` / `修改、废止的决定` 等） |
| `external_id` | str \| None | 来源系统的文档 ID（标识**版本**，不是法律本身） |
| `source_code` | str \| None | 来源标识（当前语料为 `national_laws`） |
| `meta_source` | str | 三个身份字段（`law_name` / `issuing_authority` / `publish_date`）的来源：`rule` / `docx-props` / `rule+docx`；与 `has_toc` 同属排查字段 |

**文档级字段的来源与优先级**：`law_name` / `issuing_authority` / `publish_date` 三个
身份字段优先取 docx 内嵌属性（`docProps/custom.xml`，`pipeline/docx_meta.py` 读取），
逐字段回退到正文解析。`publish_date` 两个来源语义不同——正文括注的第一个日期是
...
本期不做时效过滤——**只召回现行有效版本靠入库侧选版解决，不在检索侧过滤**。

> **2026-09-15 / 09-16 更新（三次）**：先是因"`validity_status` 枚举定义拿不到"决定 F-103 不做；
> 随后**该定义在数据源字典里找到了**（`3` 现行有效 / `2` 已修改 / `1` 已废止 / `-1` 已失效 /
> `4` 尚未生效 / `0` 未标注，见
> [legal-first-ingest-checklist.md](legal-first-ingest-checklist.md) 第四节），前提不再成立。
>
> 接着**改成"默认排除 + 参数放开"**（排除 `1` 已废止与 `-1` 已失效，`include_invalid` 放开）。
>
> **再改为：不做状态过滤，把状态发出去由调用方判断。** 入库口径始终不变——指示明确是"入库不受
> 影响"，已废止/已失效的法条照收（共约 12%），保留全集也保住了"行为时法"这类查询。检索侧则
> **六种取值一律返回**，并在每条结果的 `metadata` 里附 `validity_status`（原值）与
> `validity_status_label`（中文描述）。要只看现行有效、或排除某几种，由调用方按这两个字段自己
> 判断。
>
> 取消过滤的两条理由：一是**过滤的口径本来就该由调用方定**（"还有个 2027 年生效的新版"这种场景
...
- **Milvus**：**不新增任何字段**（见 D1）。仅写入路径的 `content` 前缀有变化
  ...
  > **2026-09-14 复核**：新到的《法条检索基础API PRD》要求按效力层级与省份/城市过滤，D1
  > 因此需要重新决策。由于 Milvus 是固定 schema 且事后补值必须重算向量，这项必须在首次
  > 入库前定下来。前置清单见 [`docs/legal-first-ingest-checklist.md`](legal-first-ingest-checklist.md)，
  > 决策记录见 `.agents/notes/proposed/architecture/2026-09-14-legal-filter-scalars-before-first-ingest.md`。
...
## 16. PRD 验收标准走查（2026-09-15）

对着《法条检索基础API PRD v0.1》的第 6 节验收标准逐项跑。**跑在本地环境**（新建镜像，
库里 9 份法条：法律 2、司法解释 2、地方法规 4、修改废止决定 1），原文用例里涉及的
《刑法》《民法典》尚未入库——全量入库还在测试环境跑（22,037 份），因此凡涉及这两部法的用例
都用**同形态的等价用例**替代并注明。
...
| 验收项 | 结论 | 实测证据 |
|---|---|---|
| 关键词检索 | **等价用例通过** | 「世界遗产 保护管理 协调机制」→ top-1《泉州：宋元中国的世界海洋商贸中心》第六条。原文用例（"交通肇事"→《刑法》第133条）待满库复跑 |
| 编号精确检索 | **等价用例通过，且优于预期** | `match_mode=exact` + 「“景德镇制”陶瓷保护条例 第六条」→ 恰好 1 条、`mode=exact`、`routes=["exact"]`、**0.1 秒**（同一查询走 semantic 要 20–30 秒，因 exact 不走向量召回） |
| 层级筛选 - 仅法律 | **通过（字面）** | 同一查询加 `law_levels=["law"]` → 返回 8 条全是 `law_type=法律`，**0 条司法解释**；加 `["judicial_interpretation"]` → 8 条全是司法解释 |
| 位阶排序 | **不通过（按字面）** | 见下方"待确认口径"。同一查询里 法律(tier 0.9) 排 #4、司法解释(tier 0.7) 排 #1 |
| 地域筛选 | **等价用例通过** | 「陶瓷 地理标志 专用标识」+ 江西省/景德镇市 → 5/5 命中景德镇条例；把过滤换成 `province=北京市` → 该条例**被完全排除**，只剩国家层面法规 |

### 6.2 法条内容校验（PRD 表 6）

| 验收项 | 结论 | 实测证据 |
|---|---|---|
| 内容匹配 | **等价用例通过** | 用第十六条正文片段（不带条号）作查询 → top-1 正是第十六条 |
| 编号错误容忍 | **等价用例通过** | 查询里故意带库里不存在的「第九十九条」+ 真实正文 → top-1 仍是第十六条 |
| 仅返回有效法条 | **口径已改，不由服务端做** | 检索返回全部状态，并附 `validity_status` 与 `validity_status_label`（实测：已废止文档照常返回且标签为「已废止」，默认口径不再剔除任何取值） |

### 6.3 数据准确性（PRD 表 7）

| 验收项 | 结论 | 实测证据 |
|---|---|---|
| 原文一致性 | **通过** | 新详情端点 `GET /api/legal/articles/{id}` 返回的第六条正文，与原件经 `DocxLoader` + `LawsChunker(child_policy="article")` 抽出的该条**逐字节一致**（205 字） |
| 有效性 | **由调用方判断** | 同上：状态随结果下发（原值 + 中文），六种取值都会返回 |

### 走查得出的三件事

**一、位阶排序的口径要产品定（唯一未通过项）。** 现在是"相关度为主、位阶为辅"：复合分乘
`(1 + 0.1 × 位阶分)`。实测做了一次真 A/B（改平台配置 `legal_level_weight` 0.1 → 0 后重启）：
分数确实按层级变了（司法解释 0.4148 → 0.3948、法律 0.2136 → 0.2019），但**排序 20 条一条没动**——
相关度差距远大于权重能补的量。PRD 那句"法律层级结果排在司法解释之前"如果按字面读，意思是
**层级优先的分层排序**（所有法律排在所有司法解释之前），那是另一套排序策略，且要以牺牲相关度为
代价。需要产品在"相关度优先、位阶为辅"（现状）与"层级优先"之间明确选一个。

**二、返回顺序不等于 `score` 降序，要写进接口文档。** 实测同一结果里 `#5 score=0.1945` 排在
`#6 score=0.2567` 之前——因为链路末段有 MMR（多样性重排）。下游若按 `score` 自行重排会得到
不同顺序。这是既有行为，不是本次改动引入的，但此前没写在对外文档里。

**三、exact 模式的价值被实测放大。** 它不只是"返回得准"（恰好 1 条），还把延迟从 20–30 秒降到
0.1 秒——因为它按元数据直接定位，不走向量召回与 rerank。对上游 LLM 做程序化校验的场景，这个
差距比准确率更影响可用性。
